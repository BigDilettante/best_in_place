...
BestInPlaceEditor.prototype = {
    ...
    update: function () {
        ...
        switch (this.formType) {
            ...
            case "multiselect":
                ...
                var outputValues = [];
                $.each(this.values, function(index, arr) {
                    if (value.indexOf(String(arr[0])) > -1) {
                        outputValues.push(arr[1]);
                    }
                });
                editor.element.html(outputValues.join(", "));
                break;
    ...
    initOptions: function () {
        ...
        self.useChosen = self.element.data("bipUseChosen") || self.useChosen || BestInPlaceEditor.defaults.useChosen
        ...
        if (self.formType === "select" || self.formType == "multiselect" || self.formType === "checkbox") {
    ...
    initPlaceHolder: function () {
        ...
        if (this.element.html() === "" || this.element.html() === "<p></p>") {
            this.element.html(this.placeHolder);
        }
    },
    ...
    requestData: function () {
        ...
        var return_data = this.getValue();
        if(return_data instanceof Array) {
          var x;
            if(return_data.length == 0) {
                data += "&" + this.objectName + '[' + this.attributeName + ']=[]';
            }
            else {
                for(x=0;x<return_data.length;x++) {
                  return_data[x] = this.objectName + '[' + this.attributeName + '][]=' + encodeURIComponent(return_data[x]);
                }
          ...
          return_data = encodeURIComponent(return_data);
          data += "&" + this.objectName + '[' + this.attributeName + ']=' + return_data;
        }
    ...
    loadSuccessCallback: function (data, status, xhr) {
        ...
        if (this.collectionValue !== null && (this.formType == "select" || this.formType == "multiselect")) {
...
BestInPlaceEditor.forms = {
    ...
    "multiselect" : {
      activateForm : function() {
        var output     = jQuery(document.createElement('form'))
          ...
          if (this.collectionValue instanceof Array) {
              currentCollectionValue = this.collectionValue;
          } else {
              try {
                  currentCollectionValue = jQuery.parseJSON(this.collectionValue);
              } catch (x) {
                  if(this.collectionValue != undefined) {
                      currentCollectionValue = this.collectionValue.split(',');
                  }
              }
          }
        ...
        jQuery.each(this.values, function (index, value) {
          var option_elt = jQuery(document.createElement('option'))
                           ...
                           .val(value[0])
                           .html(value[1]);
          if(currentCollectionValue.indexOf(value[0]) > -1 || currentCollectionValue.indexOf(String(value[0])) > -1) {
            option_elt.attr('selected', 'selected');
          }
          select_elt.append(option_elt);
        });
        ...
        if(this.useChosen == true || typeof this.useChosen == 'object') {
            var _this = this;
            select_elt.chosen(this.useChosen).change(function(e) {
                e.data = {editor: _this};
                BestInPlaceEditor.forms.multiselect.submitHandler(e);
            });
        }
        ...
        // this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.multiselect.submitHandler);

        // if (this.cancelButton) {
        //     this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.multiselect.cancelButtonHandler);
        // }

        if(typeof this.useChosen == 'object') {
            var $chosenEl = this.element.find("div.chosen-container");
            var _this = this;
            var $chosenInput = $chosenEl.find('input[type=text]')
            $(document).bind('keyup', {editor: this}, BestInPlaceEditor.forms.multiselect.keyupHandler);
            $chosenInput.bind('blur', {editor: this}, BestInPlaceEditor.forms.multiselect.blurDesktopHandler);
            $chosenInput.focus();
        }
        else {
            select_elt.bind('blur', {editor: this}, BestInPlaceEditor.forms.multiselect.blurTabletHandler);
            select_elt.bind('keyup', {editor: this}, BestInPlaceEditor.forms.multiselect.keyupHandler);
            select_elt[0].focus();
        }
      },
      ...
      getValue : function() {
        ...
        var return_array = [];
        var select_elem;
        if (select_elem = this.element.find("select")[0]) {
            for (x=0;x < select_elem.length;x++) {
                if(select_elem[x].selected) {
                    v = this.sanitizeValue(select_elem[x].value);
                    return_array.push(v);
                }
            }
        }
      ...
      blurDesktopHandler : function(event) {
        'use strict';
        event.data.editor.blurTimer = setTimeout(function () {
            if (!event.data.editor.userClicked) {
                event.data.editor.update();
                event.data.editor.element.trigger(jQuery.Event("best_in_place:deactivate"));
            }
        }, 200);
      },

      blurTabletHandler : function(event) {
        'use strict';
        event.data.editor.update();
        event.data.editor.element.trigger(jQuery.Event("best_in_place:deactivate"));
      },
      ...
      submitHandler: function (event) {
          ...
          // event.data.editor.element.trigger(jQuery.Event("best_in_place:deactivate"));
          setTimeout(function() {
            event.data.editor.userClicked = false;
          }, 250);
      },
    ...
    "textarea": {
        ...
        submitHandler: function (event) {
            ...
            event.data.editor.update();
            setTimeout(function () {
                event.data.editor.initPlaceHolder();
            }, 250);
        },
