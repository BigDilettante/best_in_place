...
BestInPlaceEditor.forms = {
    ...
    "select": {
        activateForm: function () {
            ...
            if(this.useChosen == true || typeof this.useChosen == 'object') {
                select_elt.chosen(this.useChosen);
            }
            ...
            if(typeof this.useChosen == 'object') {
                var chosen_elt = this.element.find("div.chosen-container");
                var _this = this;
                $(document).bind('click', {editor: this}, function(e) {
                    var clicked = $(e.target)
                    if(clicked.closest(_this.element).length || clicked.is(_this.element) || clicked.hasClass("best_in_placeholder")) {
                        return;
                    }
                    else {
                        BestInPlaceEditor.forms.select.blurHandler(e)
                    }
                });
                $(document).bind('keyup', {editor: this}, BestInPlaceEditor.forms.select.keyupHandler);
                chosen_elt.find('a.chosen-single').click();
            }
            else {
    ...
    "multiselect" : {
      activateForm : function() {
        ...
        jQuery.each(this.values, function (index, value) {
          var option_elt = jQuery(document.createElement('option'))
                           ...
                           .val(value[0])
                           .html(value[1]);
          if(currentCollectionValue.indexOf(value[0]) > -1) {
        ...
        if (this.cancelButton) {
            console.log(this.element.find("input[type='button']"));
        ...
        if(typeof this.useChosen == 'object') {
            ...
            $(document).bind('click', {editor: this}, function(e) {
                ...
                if(clicked.closest(_this.element).length || clicked.is(_this.element) || clicked.hasClass("best_in_placeholder")) {
                    ...
                    BestInPlaceEditor.forms.multiselect.blurHandler(e)
                }
            });
            $(document).bind('keyup', {editor: this}, BestInPlaceEditor.forms.multiselect.keyupHandler);
            chosen_elt.find('input[type=text]').focus();
        }
        else {
      ...
      getValue : function() {
        ...
        var x;
        var return_array = [];
        var select_elem = this.element.find("select")[0];
        for (x=0;x < select_elem.length;x++) {
            if(select_elem[x].selected) {
                return_array.push(this.sanitizeValue(select_elem[x].value));
            }
        }
        return return_array;
      },
