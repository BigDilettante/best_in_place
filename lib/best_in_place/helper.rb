  ...
  module Helper
    def best_in_place(object, field, opts = {})
      ...
      if opts[:collection] or type == :checkbox
        ...
        if type == :multiselect
          display_value = collection.flat_map { |a| value && value.include?(a[0]) ? a[1] : nil }.compact.join(", ")
        else
          value = value.to_s
      ...
      options[:data]['bip-use-chosen'] = opts[:use_chosen]
